...
export const createAcceso = async (req, res) => {
  try {
    ...
    // ---------------------------
    // Procesar Credenciales_DB
    // ---------------------------
    const isDBEnabled = EstadoCredencialDB === "on";
    const userCredencialDB = isDBEnabled ? UsuarioCredencialDB : "No aplica";
    const passwordCredencialDB = isDBEnabled ? ContrasenaEncriptadaDB : "No aplica";

    // ---------------------------
    // Procesar Credenciales_App
    // ---------------------------
    const isAppEnabled = EstadoCredencialApp === "on";
    const userCredencialApp = isAppEnabled ? UsuarioCredencialApp : "No aplica";
    const passwordCredencialApp = isAppEnabled ? ContrasenaEncriptadaApp : "No aplica";
    ...
    const servidorResult = await connection
      .request()
      ...
      .input("Ambiente", sql.Int, Ambiente).query(`
  ...
  `);
    ...
    const id_servidor = await servidorResult.recordset[0].id_servidor;
    ...
    try {
      await connection
        ...
        .input("estado", sql.Int, EstadoCredencial).query(`
  ...
  `);
    ...
    try {
      await connection
        .request()
        .input("id_servidor", sql.Int, id_servidor)
        .input("usuario", sql.VarChar, userCredencialDB) // <-- variable procesada
        .input("contrasena_encriptada", sql.VarChar, passwordCredencialDB) // <-- variable procesada
        .input("id_categoria", sql.Int, 2)
        .input("estado", sql.Int, isDBEnabled) // <-- nuevo estado (0 o 1)
        .query(`
    ...
    try {
      await connection
        .request()
        .input("id_servidor", sql.Int, id_servidor)
        .input("usuario", sql.VarChar, userCredencialApp) // <-- variable procesada
        .input("contrasena_encriptada", sql.VarChar, passwordCredencialApp) // <-- variable procesada
        .input("id_categoria", sql.Int, 3)
        .input("estado", sql.Int, isAppEnabled) // <-- nuevo estado (0 o 1)
        .query(`
    ...
    try {
      await connection
        ...
        .input("id_sistema", sql.Int, SistemaNombre)
        .input("acceso_contacto", sql.VarChar, null).query(`
    INSERT INTO AccesoServidor (id_servidor, id_tipo_acceso, id_uunn, id_sistema, acceso_contacto)
    VALUES (@id_servidor, @id_tipo_acceso, @id_uunn, @id_sistema, @acceso_contacto)
  `);
    } catch (error) {
      console.error(error)
     }
    res.status(201).json({
      message: "Servidor y accesos registrados correctamente",
      id_servidor,
    });
  } catch (error) {
    ...
    res
      .status(500)
      .json({ message: "Error al registrar acceso", error: error.message });
  }
};
...
export const cargarAmbientes = async (req, res) => {
  try {
    ...
    const result = await pool
      .request()
      .query("SELECT id_ambiente, nombre FROM Ambientes");
...
export const cargarSistemas = async (req, res) => {
  try {
    ...
    const result = await pool
      .request()
      .query("SELECT id_sistema, nombre FROM Sistemas");

    if (result.recordset.length === 0) {
      return res.status(404).json({
        success: false,
        message: "No se encontraron Sistemas Informaticos",
      });
    }
...
export const cargarTipoConexion = async (req, res) => {
  try {
    ...
    const result = await pool
      .request()
      .query("SELECT id_tipo_acceso, nombre FROM TipoAcceso");

    if (result.recordset.length === 0) {
      return res.status(404).json({
        success: false,
        message: "No se encontraron tipos de accesos",
      });
    }
...
export const cargarUnidadNegocio = async (req, res) => {
  try {
    ...
    const result = await pool
      .request()
      .query("SELECT id_uunn, nombre FROM UnidadNegocio");
    if (result.recordset.length === 0) {
      return res.status(404).json({
        success: false,
        message: "No se encontraron Unidades de Negocios",
      });
    }
...
export const getAccesos = async (req, res) => {
  try {
    ...
    const result = await connection
      .request()
      .query(`EXEC dbo.sp_ConsultarAccesosServidores`);
...
export const getAcceso = async (req, res) => {
  ...
  try {
    const connection = await getConnection();
    const result = await connection.request().input("id_acceso", id).query(`
        SELECT 
          asv.id_acceso,
    ...
    res
      .status(500)
      .json({ message: "Error al obtener el acceso", error: error.message });
  }
};
...
export const updateAcceso = async (req, res) => {
  ...
  try {
    ...
    const result = await connection
      ...
      .input("acceso_contacto", acceso_contacto).query(`
    ...
    res
      .status(200)
      .json({ message: "AccesoServidor actualizado correctamente" });
  } catch (error) {
    console.error(error);
    res.status(500).json({
      message: "Error al actualizar AccesoServidor",
      error: error.message,
    });
  }
};
...
export const deleteAcceso = async (req, res) => {
  ...
  try {
    ...
    console.error(error);
    res.status(500).json({
      message: "Error al eliminar AccesoServidor",
      error: error.message,
    });
  }
};
